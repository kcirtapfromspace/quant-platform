  ...
  const runBacktest = useCallback(async () => {
    ...
    try {
      const data = await apiFetch<BacktestResult>(
        `/backtest/${encodeURIComponent(sym)}?range=${range}&signal=${signal}`,
      );
      setResult(data);
    } catch (e: unknown) {
      ...
      {result && !loading && (
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2">
            ...
            <Metric label="Sharpe" value={fmtNum(result.sharpeRatio)} positive={result.sharpeRatio >= 1} />
            <Metric label="Max Drawdown" value={fmtPct(-result.maxDrawdown)} positive={false} />
            <Metric label="Win Rate" value={fmtPct(result.winRate, 1)} positive={result.winRate >= 0.5} />
            <Metric label="Profit Factor" value={result.profitFactor >= 99 ? '∞' : fmtNum(result.profitFactor, 2)} positive={result.profitFactor >= 1} />
            <Metric label="Trades" value={String(result.tradeCount)} positive={null} />
          </div>
          ...
          <div className="text-xs text-slate-600 pb-2">
            Buy-and-hold simulation · commission 10 bps · initial capital $1M · data via Yahoo Finance
          </div>
        </>
      )}

...
export function AnalyticsPage() {
  ...
  const fetchAnalytics = useCallback(async (sym: string, rng: Range, bench: string) => {
    ...
    try {
      const [perf, attr] = await Promise.all([
        apiFetch<PerformanceData>(
          `/analytics/performance?symbol=${encodeURIComponent(sym)}&range=${rng}&benchmark=${bench}`,
        ),
        apiFetch<AttributionRow[]>('/analytics/attribution'),
      ]);
      setPerfData(perf);
      setAttribution(attr);
    } catch (e: unknown) {
  ...
  useEffect(() => {
    void fetchAnalytics(symbol, range, benchmark);
  }, []);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!sym) return;
    setSymbol(sym);
    void fetchAnalytics(sym, range, benchmark);
  };

  const handleRangeChange = (r: Range) => {
    setRange(r);
    void fetchAnalytics(symbol, r, benchmark);
  };

  const handleBenchmarkChange = (b: string) => {
    setBenchmark(b);
    void fetchAnalytics(symbol, range, b);
  };

...
function generateMockHistory(days = 90): Array<{ time: number; value: number }> {
  const now = Date.now();
  const MS_PER_DAY = 86_400_000;
  let nav = 1_000_000;
  const out: Array<{ time: number; value: number }> = [];
  for (let i = days; i >= 0; i--) {
    nav *= 1 + (Math.random() - 0.48) * 0.012;
    out.push({ time: Math.floor((now - i * MS_PER_DAY) / 1000), value: Math.round(nav * 100) / 100 });
  }
  return out;
}
...
function ExposureChart({ portfolio }: ExposureChartProps) {
...
function StatCard({ label, value, subValue, valueClass = 'text-slate-100' }: StatCardProps) {
  return (
...
export function PortfolioDashboard({ portfolio }: PortfolioDashboardProps) {
  const [historyData, setHistoryData] = useState<Array<{ time: number; value: number }>>([]);

  useEffect(() => {
    fetch('/api/portfolio/history')
      .then((r) => r.json())
      .then((data: Array<{ time: number; value: number }>) => {
        if (Array.isArray(data) && data.length > 0) setHistoryData(data);
        else setHistoryData(generateMockHistory());
      })
      .catch(() => setHistoryData(generateMockHistory()));
  }, []);

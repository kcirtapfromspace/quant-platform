...
function CategoryPill({ category }: { category: StrategyCategory }) {
  ...
  return <span className={`text-xs font-mono ${cfg[category]}`}>{category}</span>;
}
...
function StrategyCard({ strategy, onToggle }: { strategy: StrategyState; onToggle: (key: string) => void }) {
...
function ConfirmModal({ strategyName, action, onConfirm, onCancel }: {
  strategyName: string;
  action: 'halt' | 'live';
  onConfirm: () => void;
  onCancel: () => void;
...
export function StrategyMonitorPage() {
  ...
  const { data: restStrategies } = useStrategies();
  ...
  useEffect(() => {
    if (!restStrategies || localStates.size > 0) return;
    const m = new Map<string, StrategyState>();
    for (const s of restStrategies) m.set(s.strategy_key, s);
    setLocalStates(m);
  }, [restStrategies]);
    ...
    <div className="flex flex-col gap-4 p-4 min-h-0 overflow-y-auto">
      ...
      {strategies.length === 0 && (
        <div className="flex items-center justify-center py-16 text-slate-500 text-sm">
          {connected ? 'Waiting for strategy data…' : 'Connecting to backend…'}
        </div>
      )}
